document.addEventListener("DOMContentLoaded", function () {
    ...
    let banquesTable = new DataTable("#banquesTable", {
        // dom: "Bfrtip",
    ...
    banquesTable.on("select", function (e, dt, type, indexes) {
        let checked = banquesTable.rows(indexes).data()[0];

        let banqueIdChecked = $(checked[0])
            .find(".form-check-input")
            .val();

        banqueSelected.push(banqueIdChecked);
        console.log(banqueSelected);
    });

    banquesTable.on("deselect", function (e, dt, type, indexes) {
        let checked = banquesTable.rows(indexes).data()[0];
        ...
        banqueSelected = banqueSelected.filter(
            (input) => input !== banqueIdChecked
        );
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        url: route("delete-banques"),
                        data: {
                            banques_ids: banqueSelected,
                        },
