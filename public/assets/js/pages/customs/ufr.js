document.addEventListener("DOMContentLoaded", function () {
    ...
    let ufrsTable = new DataTable("#ufrsTable", {
        // dom: "Bfrtip",
    ...
    ufrsTable.on("select", function (e, dt, type, indexes) {
        let checked = ufrsTable.rows(indexes).data()[0];

        let ufrIdChecked = $(checked[0])
            .find(".form-check-input")
            .val();

        ufrSelected.push(ufrIdChecked);
        console.log(ufrSelected);
    });

    ufrsTable.on("deselect", function (e, dt, type, indexes) {
        let checked = ufrsTable.rows(indexes).data()[0];
        ...
        ufrSelected = ufrSelected.filter(
            (input) => input !== ufrIdChecked
        );
    ...
    //                 ufrSelected.push($(checkbox.val()));
    //                 // checkbox.closest("tr").classList.add("table-active");
    //                 ufrsTable.rows().select();
    //             });
    //         } else {
    //             Array.from(checkboxes).forEach(function (checkbox) {
    //                 checkbox.checked = false;
    //                 // checkbox.closest("tr").classList.remove("table-active");
    //                 ufrsTable.rows().deselect();
    //             });
    //         }
    //     };
    // }
    ...
    $("#delete-record").on("click", function (e) {
        e.preventDefault();
        if (ufrSelected.length === 0) {
            Swal.fire({
                title: "Selectionner au moins une ufr!",
                text: "",
                icon: "warning",
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        url: route("delete-ufrs"),
                        data: {
                            ufrs_ids: ufrSelected,
                        },
