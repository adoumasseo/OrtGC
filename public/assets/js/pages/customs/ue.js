document.addEventListener("DOMContentLoaded", function () {
    ...
    let uesTable = new DataTable("#uesTable", {
        // dom: "Bfrtip",
    ...
    uesTable.on("select", function (e, dt, type, indexes) {
        let checked = uesTable.rows(indexes).data()[0];

        let ueIdChecked = $(checked[0])
            .find(".form-check-input")
            .val();

        ueSelected.push(ueIdChecked);
        console.log(ueSelected);
    });

    uesTable.on("deselect", function (e, dt, type, indexes) {
        let checked = uesTable.rows(indexes).data()[0];
        ...
        ueSelected = ueSelected.filter(
            (input) => input !== ueIdChecked
        );
    ...
    //                 ueSelected.push($(checkbox.val()));
    //                 // checkbox.closest("tr").classList.add("table-active");
    //                 uesTable.rows().select();
    //             });
    //         } else {
    //             Array.from(checkboxes).forEach(function (checkbox) {
    //                 checkbox.checked = false;
    //                 // checkbox.closest("tr").classList.remove("table-active");
    //                 uesTable.rows().deselect();
    //             });
    //         }
    //     };
    // }
    ...
    $("#delete-record").on("click", function (e) {
        e.preventDefault();
        if (ueSelected.length === 0) {
            Swal.fire({
                title: "Selectionner au moins une ue!",
                text: "",
                icon: "warning",
